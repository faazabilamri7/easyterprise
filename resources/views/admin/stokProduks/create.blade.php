...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.stok-produks.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="nama_produk">{{ trans('cruds.stokProduk.fields.nama_produk') }}</label>
                <input class="form-control {{ $errors->has('nama_produk') ? 'is-invalid' : '' }}" type="text" name="nama_produk" id="nama_produk" value="{{ old('nama_produk', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('biaya_produksi') ? 'is-invalid' : '' }}" type="number" name="biaya_produksi" id="biaya_produksi" value="{{ old('biaya_produksi', '') }}" step="0.01">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\StokProduk::KATEGORI_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('kategori') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="kategori_{{ $key }}" name="kategori" value="{{ $key }}" {{ old('kategori', '') === (string) $key ? 'checked' : '' }}>
