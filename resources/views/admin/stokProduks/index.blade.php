...
@can('stok_produk_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.stok-produks.create') }}">
                        ...
                        <tr data-entry-id="{{ $stokProduk->id }}">
                            ...
                            <td>
                                @can('stok_produk_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.stok-produks.show', $stokProduk->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('stok_produk_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.stok-produks.edit', $stokProduk->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('stok_produk_delete')
                                    <form action="{{ route('admin.stok-produks.destroy', $stokProduk->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('stok_produk_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.stok-produks.massDestroy') }}",
