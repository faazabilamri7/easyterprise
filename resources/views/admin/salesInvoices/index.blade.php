                        ...
                        <tr data-entry-id="{{ $salesInvoice->id }}">
                            ...
                            <td>
                                @can('sales_invoice_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.sales-invoices.show', $salesInvoice->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('sales_invoice_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.sales-invoices.edit', $salesInvoice->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('sales_invoice_delete')
                                    <form action="{{ route('admin.sales-invoices.destroy', $salesInvoice->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
