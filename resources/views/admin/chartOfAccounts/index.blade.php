...
@can('chart_of_account_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.chart-of-accounts.create') }}">
                        ...
                        <tr data-entry-id="{{ $chartOfAccount->id }}">
                            ...
                            <td>
                                @can('chart_of_account_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.chart-of-accounts.show', $chartOfAccount->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('chart_of_account_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.chart-of-accounts.edit', $chartOfAccount->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('chart_of_account_delete')
                                    <form action="{{ route('admin.chart-of-accounts.destroy', $chartOfAccount->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('chart_of_account_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.chart-of-accounts.massDestroy') }}",
