<div class="m-3">
    @can('invoice_pembelian_create')
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                <a class="btn btn-success" href="{{ route('admin.invoice-pembelians.create') }}">
    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class=" table table-bordered table-striped table-hover datatable datatable-perusahaanInvoicePembelians">
                    <thead>
                        <tr>
                            <th width="10">
                            ...
                            <tr data-entry-id="{{ $invoicePembelian->id }}">
                                ...
                                <td>
                                    @can('invoice_pembelian_show')
                                        <a class="btn btn-xs btn-primary" href="{{ route('admin.invoice-pembelians.show', $invoicePembelian->id) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan

                                    @can('invoice_pembelian_edit')
                                        <a class="btn btn-xs btn-info" href="{{ route('admin.invoice-pembelians.edit', $invoicePembelian->id) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan

                                    @can('invoice_pembelian_delete')
                                        <form action="{{ route('admin.invoice-pembelians.destroy', $invoicePembelian->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('invoice_pembelian_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.invoice-pembelians.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-perusahaanInvoicePembelians:not(.ajaxTable)').DataTable({ buttons: dtButtons })
