...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.transfer-produks.update", [$transferProduk->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="nama_produk">{{ trans('cruds.transferProduk.fields.nama_produk') }}</label>
                <input class="form-control {{ $errors->has('nama_produk') ? 'is-invalid' : '' }}" type="text" name="nama_produk" id="nama_produk" value="{{ old('nama_produk', $transferProduk->nama_produk) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('transfer_dari') ? 'is-invalid' : '' }}" name="transfer_dari" id="transfer_dari">
                    ...
                    @foreach(App\Models\TransferProduk::TRANSFER_DARI_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('transfer_dari', $transferProduk->transfer_dari) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('transfer_ke') ? 'is-invalid' : '' }}" name="transfer_ke" id="transfer_ke">
                    ...
                    @foreach(App\Models\TransferProduk::TRANSFER_KE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('transfer_ke', $transferProduk->transfer_ke) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
