...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.stok-materials.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="nama_material">{{ trans('cruds.stokMaterial.fields.nama_material') }}</label>
                <input class="form-control {{ $errors->has('nama_material') ? 'is-invalid' : '' }}" type="text" name="nama_material" id="nama_material" value="{{ old('nama_material', '') }}">
