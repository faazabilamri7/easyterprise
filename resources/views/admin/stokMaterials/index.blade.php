...
@can('stok_material_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.stok-materials.create') }}">
                        ...
                        <tr data-entry-id="{{ $stokMaterial->id }}">
                            ...
                            <td>
                                @can('stok_material_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.stok-materials.show', $stokMaterial->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('stok_material_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.stok-materials.edit', $stokMaterial->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('stok_material_delete')
                                    <form action="{{ route('admin.stok-materials.destroy', $stokMaterial->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('stok_material_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.stok-materials.massDestroy') }}",
