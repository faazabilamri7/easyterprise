...
@can('vendor_status_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.vendor-statuses.create') }}">
                        ...
                        <tr data-entry-id="{{ $vendorStatus->id }}">
                            ...
                            <td>
                                @can('vendor_status_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.vendor-statuses.show', $vendorStatus->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('vendor_status_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.vendor-statuses.edit', $vendorStatus->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('vendor_status_delete')
                                    <form action="{{ route('admin.vendor-statuses.destroy', $vendorStatus->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('vendor_status_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.vendor-statuses.massDestroy') }}",
