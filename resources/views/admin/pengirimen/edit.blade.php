    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.pengirimen.update", [$pengiriman->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('id_shipment') ? 'is-invalid' : '' }}" type="text" name="id_shipment" id="id_shipment" value="{{ old('id_shipment', $pengiriman->id_shipment) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status_shipment') ? 'is-invalid' : '' }}" name="status_shipment" id="status_shipment">
                    ...
                    @foreach(App\Models\Pengiriman::STATUS_SHIPMENT_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status_shipment', $pengiriman->status_shipment) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
