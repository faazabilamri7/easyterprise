...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.material-entries.update", [$materialEntry->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="material_name_id">{{ trans('cruds.materialEntry.fields.material_name') }}</label>
                <select class="form-control select2 {{ $errors->has('material_name') ? 'is-invalid' : '' }}" name="material_name_id" id="material_name_id" required>
                    @foreach($material_names as $id => $entry)
                        <option value="{{ $id }}" {{ (old('material_name_id') ? old('material_name_id') : $materialEntry->material_name->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
