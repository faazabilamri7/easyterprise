        ...
        <div class="col-md-12">
            @can('akun_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.akuns.create') }}">
                            ...
                            <tbody>
                                @foreach($akuns as $key => $akun)
                                    <tr data-entry-id="{{ $akun->id }}">
                                        ...
                                        <td>
                                            @can('akun_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.akuns.show', $akun->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('akun_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.akuns.edit', $akun->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('akun_delete')
                                                <form action="{{ route('frontend.akuns.destroy', $akun->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('akun_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.akuns.massDestroy') }}",
