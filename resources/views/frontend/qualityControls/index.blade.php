                                    ...
                                    <tr data-entry-id="{{ $qualityControl->id }}">
                                        ...
                                        <td>
                                            @can('quality_control_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.quality-controls.show', $qualityControl->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('quality_control_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.quality-controls.edit', $qualityControl->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('quality_control_delete')
                                                <form action="{{ route('frontend.quality-controls.destroy', $qualityControl->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('quality_control_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.quality-controls.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-QualityControl:not(.ajaxTable)').DataTable({ buttons: dtButtons })
