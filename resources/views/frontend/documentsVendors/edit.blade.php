        ...
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-header">
                    {{ trans('global.edit') }} {{ trans('cruds.documentsVendor.title_singular') }}
                </div>

                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.documents-vendors.update", [$documentsVendor->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="vendor_id" id="vendor_id" required>
                                @foreach($vendors as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('vendor_id') ? old('vendor_id') : $documentsVendor->vendor->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $documentsVendor->name) }}">
                            @if($errors->has('name'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('name') }}
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="description" id="description">{{ old('description', $documentsVendor->description) }}</textarea>
...
<script>
    ...
    url: '{{ route('frontend.documents-vendors.storeMedia') }}',
    maxFilesize: 2, // MB
    maxFiles: 1,
    addRemoveLinks: true,
    headers: {
    ...
    init: function () {
@if(isset($documentsVendor) && $documentsVendor->document_file)
      var file = {!! json_encode($documentsVendor->document_file) !!}
