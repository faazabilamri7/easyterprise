        ...
        <div class="col-md-12">
            @can('jurnal_penyelesaian_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.jurnal-penyelesaians.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $jurnalPenyelesaian->id }}">
                                        ...
                                        <td>
                                            @can('jurnal_penyelesaian_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.jurnal-penyelesaians.show', $jurnalPenyelesaian->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('jurnal_penyelesaian_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.jurnal-penyelesaians.edit', $jurnalPenyelesaian->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('jurnal_penyelesaian_delete')
                                                <form action="{{ route('frontend.jurnal-penyelesaians.destroy', $jurnalPenyelesaian->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('jurnal_penyelesaian_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.jurnal-penyelesaians.massDestroy') }}",
