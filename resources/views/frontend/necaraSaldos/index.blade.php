        ...
        <div class="col-md-12">
            @can('necara_saldo_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.necara-saldos.create') }}">
                            {{ trans('global.add') }} {{ trans('cruds.necaraSaldo.title_singular') }}
                        </a>
                        <button class="btn btn-warning" data-toggle="modal" data-target="#csvImportModal">
                            {{ trans('global.app_csvImport') }}
                        </button>
                        @include('csvImport.modal', ['model' => 'NecaraSaldo', 'route' => 'admin.necara-saldos.parseCsvImport'])
                    </div>
                </div>
            @endcan
            <div class="card">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class=" table table-bordered table-striped table-hover datatable datatable-NecaraSaldo">
                            <thead>
                                <tr>
                                    <th>
                                    ...
                                    <tr data-entry-id="{{ $necaraSaldo->id }}">
                                        ...
                                        <td>
                                            @can('necara_saldo_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.necara-saldos.show', $necaraSaldo->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('necara_saldo_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.necara-saldos.edit', $necaraSaldo->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('necara_saldo_delete')
                                                <form action="{{ route('frontend.necara-saldos.destroy', $necaraSaldo->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('necara_saldo_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.necara-saldos.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-NecaraSaldo:not(.ajaxTable)').DataTable({ buttons: dtButtons })
