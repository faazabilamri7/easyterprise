        ...
        <div class="col-md-12">
            @can('purchase_requition_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.purchase-requitions.create') }}">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class=" table table-bordered table-striped table-hover datatable datatable-PurchaseRequition">
                            <thead>
                                <tr>
                                    <th>
                                    ...
                                    <tr data-entry-id="{{ $purchaseRequition->id }}">
                                        ...
                                        <td>
                                            @can('purchase_requition_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.purchase-requitions.show', $purchaseRequition->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('purchase_requition_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.purchase-requitions.edit', $purchaseRequition->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('purchase_requition_delete')
                                                <form action="{{ route('frontend.purchase-requitions.destroy', $purchaseRequition->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('purchase_requition_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.purchase-requitions.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-PurchaseRequition:not(.ajaxTable)').DataTable({ buttons: dtButtons })
