            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class=" table table-bordered table-striped table-hover datatable datatable-SalesOrder">
                            <thead>
                                <tr>
                                    <th>
                                    ...
                                    <tr data-entry-id="{{ $salesOrder->id }}">
                                        ...
                                        <td>
                                            @can('sales_order_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.sales-orders.show', $salesOrder->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('sales_order_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.sales-orders.edit', $salesOrder->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('sales_order_delete')
                                                <form action="{{ route('frontend.sales-orders.destroy', $salesOrder->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('sales_order_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.sales-orders.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-SalesOrder:not(.ajaxTable)').DataTable({ buttons: dtButtons })
