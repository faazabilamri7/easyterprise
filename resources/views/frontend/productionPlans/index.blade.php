        ...
        <div class="col-md-12">
            @can('production_plan_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.production-plans.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $productionPlan->id }}">
                                        ...
                                        <td>
                                            @can('production_plan_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.production-plans.show', $productionPlan->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('production_plan_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.production-plans.edit', $productionPlan->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('production_plan_delete')
                                                <form action="{{ route('frontend.production-plans.destroy', $productionPlan->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('production_plan_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.production-plans.massDestroy') }}",
