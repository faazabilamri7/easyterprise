        ...
        <div class="col-md-12">
            @can('purchas_req_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.purchas-reqs.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $purchasReq->id }}">
                                        ...
                                        <td>
                                            @can('purchas_req_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.purchas-reqs.show', $purchasReq->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('purchas_req_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.purchas-reqs.edit', $purchasReq->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('purchas_req_delete')
                                                <form action="{{ route('frontend.purchas-reqs.destroy', $purchasReq->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('purchas_req_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.purchas-reqs.massDestroy') }}",
