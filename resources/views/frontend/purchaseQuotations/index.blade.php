        ...
        <div class="col-md-12">
            @can('purchase_quotation_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.purchase-quotations.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $purchaseQuotation->id }}">
                                        ...
                                        <td>
                                            @can('purchase_quotation_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.purchase-quotations.show', $purchaseQuotation->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('purchase_quotation_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.purchase-quotations.edit', $purchaseQuotation->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('purchase_quotation_delete')
                                                <form action="{{ route('frontend.purchase-quotations.destroy', $purchaseQuotation->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('purchase_quotation_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.purchase-quotations.massDestroy') }}",
