        ...
        <div class="col-md-12">
            @can('buku_besar_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.buku-besars.create') }}">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class=" table table-bordered table-striped table-hover datatable datatable-BukuBesar">
                            <thead>
                                <tr>
                                    <th>
                            ...
                            <tbody>
                                @foreach($bukuBesars as $key => $bukuBesar)
                                    <tr data-entry-id="{{ $bukuBesar->id }}">
                                        ...
                                        <td>
                                            @can('buku_besar_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.buku-besars.show', $bukuBesar->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('buku_besar_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.buku-besars.edit', $bukuBesar->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('buku_besar_delete')
                                                <form action="{{ route('frontend.buku-besars.destroy', $bukuBesar->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('buku_besar_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.buku-besars.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-BukuBesar:not(.ajaxTable)').DataTable({ buttons: dtButtons })
