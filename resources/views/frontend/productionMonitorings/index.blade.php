                                    ...
                                    <tr data-entry-id="{{ $productionMonitoring->id }}">
                                        ...
                                        <td>
                                            @can('production_monitoring_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.production-monitorings.show', $productionMonitoring->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('production_monitoring_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.production-monitorings.edit', $productionMonitoring->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('production_monitoring_delete')
                                                <form action="{{ route('frontend.production-monitorings.destroy', $productionMonitoring->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('production_monitoring_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.production-monitorings.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-ProductionMonitoring:not(.ajaxTable)').DataTable({ buttons: dtButtons })
