    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.materials.update", [$material->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            <label class="required" for="name_material">{{ trans('cruds.material.fields.name_material') }}</label>
                            <input class="form-control" type="text" name="name_material" id="name_material" value="{{ old('name_material', $material->name_material) }}" required>
                        ...
                        <div class="form-group">
                            <label class="required" for="price">{{ trans('cruds.material.fields.price') }}</label>
                            <input class="form-control" type="number" name="price" id="price" value="{{ old('price', $material->price) }}" step="0.01" required>
                            @if($errors->has('price'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('price') }}
                                </div>
                            @endif
                            <span class="help-block">{{ trans('cruds.material.fields.price_helper') }}</span>
                        </div>
                        <div class="form-group">
                            <label for="descriptive">{{ trans('cruds.material.fields.descriptive') }}</label>
                            <textarea class="form-control" name="descriptive" id="descriptive">{{ old('descriptive', $material->descriptive) }}</textarea>
...
Dropzone.options.photoDropzone = {
    url: '{{ route('frontend.materials.storeMedia') }}',
    maxFilesize: 2, // MB
